<%@ Page Language="C#" MasterPageFile="~/AdminMainPage.Master" AutoEventWireup="true" CodeBehind="SaleRegister.aspx.cs" Inherits="WebBillingSystem.SaleRegister" %>
<asp:Content id="Content1" ContentPlaceHolderid="ContentPlaceHolder1" runat="server">

            <nav class="hk-breadcrumb" aria-label="breadcrumb">
                <ol class="breadcrumb breadcrumb-light bg-transparent">
                    <li class="breadcrumb-item"><a href="#">Report</a></li>
                    <li runat="server" class="breadcrumb-item active" aria-current="page" id="breadcrumb_title">Sale Register</li>
                </ol>
            </nav>
  <!-- /Breadcrumb -->
    <div class="container">
        ...
        <div class="hk-pg-header mb-10">
            <div>
                <h4 class="hk-pg-title" runat="server" id="Header_ID"><span class="pg-title-icon"><span class="feather-icon"><i data-feather="book"></i></span></span>Sale Register</h4>
             </div>
        </div>
        <!-- /Title -->
   ...
   <div id="det_id" class="row">
                  ...
                  <div class="row">
                    <div class="col-sm-12">    
                        <!-- start Table -->                        
                        <table id="SaleRegisterTable" class="table table-hover w-100 pb-30 dataTable dtr-inline" role="grid">
                        <thead>
						        <tr>
...
<script language='javascript'>
    ...
    // main function 
     window.onload = function () {  
         ...
         $('#SaleRegisterTable').removeClass('sorting_asc');
     ...
     function account_details_display() {
         ...
         if (jsonString != '') {
             ...
             var table = $('#SaleRegisterTable').DataTable({
                 data: jsonString,
                 autoWidth: false,
                 paging: false,
                 bSortable: false,
         ...
         $('#SaleRegisterTable tbody tr').find("td:eq(0)").css("text-align", "center");
         $('#SaleRegisterTable tbody tr').find("td:eq(1)").css("text-align", "center");
         $('#SaleRegisterTable tbody tr').find("td:eq(2)").css("text-align", "right");
         $('#SaleRegisterTable tbody tr').find("td:eq(3)").css( "text-align", "right" );
         $('#SaleRegisterTable tbody tr').find("td:eq(4)").css("text-align", "right");

         $('#SaleRegisterTable tbody tr:eq(0) td:eq(0)').html("");
         $('#SaleRegisterTable tbody tr:eq(0) td:eq(1)').html("Opening Balance");
         $('#SaleRegisterTable tbody tr:eq(0) td:eq(2)').html('<label id="op_bal_dr_id" runat="server">0.00</label>');
         $('#SaleRegisterTable tbody tr:eq(0) td:eq(3)').html('<label id="op_bal_cr_id" runat="server">0.00</label>');
         $('#SaleRegisterTable tbody tr:eq(0) td:eq(4)').html("");
         
     //Format Second Table(call to this table)
     $('#SaleRegisterTable tbody').on('click', '.details-control', function() {
    ...
    //Expand and Collaps all columns
         $('#chk_all_display_dtl').on('click', function () {
            if ($('#chk_all_display_dtl').prop('checked') == false) {
                ...
                $('#SaleRegisterTable tbody tr:eq(1)').remove();
            }
         });
     // Narration onclick hide and show
         $('#chk_display_narration_id').on('click', function () {
             ...
             if ($('#chk_display_narration_id').prop('checked') == false) {
                 $("#SaleRegisterTable #note_tr_id").hide();
             } else {
                 $("#SaleRegisterTable #note_tr_id").show();
             }
         });
    ...
    function format_day(obj) {
        ...
        if (jsonStringDayDtl != '') {
            jsonStringDayDtl = JSON.parse(jsonStringDayDtl);
             ...
             var name_val_Day = '<table class="table table-hover dataTable" id="SaleRegisterTableThree"><tbody>';
    ...
    function ExportToDocMonth(element) {
        ...
        var filename = "Sale Register Month Report";
